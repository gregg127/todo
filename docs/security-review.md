...
> **Status: findings 1–3 fixed, recommendations 1–4 and 6 applied.** See
> `e0fd71d` (control characters), `aebd1dc` (the click guard), `9b058c4` (the
> dependency floor) and `31ebaa1` (CI). `govulncheck` now reports nothing at any
> level. Still open: recommendation 5 (Bubble Tea v2 migration). The findings
> below are left as written, as the record of what was wrong.
